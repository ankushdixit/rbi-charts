...
export interface Story {
  ...
  href: string;
  /** Real sparkline data — normalized 0-1 values from actual chart data */
  sparkline: number[];
  sparklineType: "area" | "bars" | "rising" | "volatile";
  /** Each card's own accent color (for sparkline + gradient bg) */
  cardColor: string;
}

/** Per-section accent color — used ONLY for the category tag text */
...
export const STORIES: Story[] = [
  // Payments Revolution — each card has its own color
  {
    slug: "upi-explosion",
